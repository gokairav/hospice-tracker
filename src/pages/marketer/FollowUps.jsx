...
export default function FollowUps() {
  ...
  return (
    <div className="px-4 py-4">
      <h1 className="text-lg font-semibold text-slate-900 mb-4">Follow-ups</h1>
      ...
      {active.length === 0 ? (
        <p className="text-sm text-slate-500 text-center py-10">No upcoming follow-ups.</p>
      ) : (
      ...
      <div className="mt-6">
        ...
        {showCompleted && (
          <ul className="mt-2 space-y-2">
            {completed.length === 0 ? (
              <p className="text-sm text-slate-400">No completed follow-ups yet.</p>
            ) : (
    ...
    <li className="bg-white rounded-xl border border-slate-200 p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <Link to={`/marketer/leads/${reminder.lead_id}`} className="font-medium text-slate-900 underline">
            {reminder.leads?.patient_first_name} {reminder.leads?.patient_last_name}
          </Link>
          <p className={`text-sm mt-0.5 ${overdue ? 'text-red-600 font-medium' : 'text-slate-500'}`}>
            {formatReminderDate(reminder.reminder_date)}
            {overdue && ' · Overdue'}
          </p>
          {reminder.note && <p className="text-sm text-slate-600 mt-1">{reminder.note}</p>}
        </div>
        {onComplete && (
          <button
            type="button"
            onClick={onComplete}
            disabled={completing}
            className="shrink-0 rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 disabled:opacity-60 active:bg-slate-50"
          >
